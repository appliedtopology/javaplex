...
public class StreamTester {
	...
	/**
	 * This function verifies that the Witness streams constructed by plex 3 and plex 4 are equal.
	 * 
	 * @param selector the selection of landmark points within a metric space
	 * @param maxDimension the maximum dimension of the complex
	 * @param maxFiltrationValue the maximum filtration value (maximum distance allowable for an edge)
	...
	public static void compareWitnessStreams(LandmarkSelector<double[]> selector, int maxDimension, double maxFiltrationValue, int numDivisions) {
		Timing.restart();
		AbstractFilteredStream<Simplex> plex3Stream = FilteredStreamInterface.createPlex3WitnessStream(selector, maxDimension, maxFiltrationValue, numDivisions);
		Timing.stopAndDisplay("Plex 3 Witness Stream");
		
		Timing.restart();
		AbstractFilteredStream<Simplex> plex4Stream = FilteredStreamInterface.createPlex4WitnessStream(selector, maxDimension, maxFiltrationValue, numDivisions);
