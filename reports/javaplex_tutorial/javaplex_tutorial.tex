...
\author{H\lc{enry} A\lc{dams and} A\lc{ndrew} T\lc{ausz} \\ \lc{henrya@math.stanford.edu and atausz@stanford.edu} \\ M\lc{ay 30, 2011}}
...
JavaPlex requires version number \texttt{1.5} or higher. \\ % CONFIRM, Answer: Confirmed

To install JavaPlex for Matlab, go the the website \url{http://code.google.com/p/javaplex/downloads/list}. Download the zip file containing the Matlab examples. It should be called something like \texttt{matlab-\-examples\-4.01.tar.gz}. % overflow
...
Your output should be the same except for the last several characters. \\% Andrew, is there a better command for users to test that their installation is working? Answer: It doesn't really matter since either it succeeds or it fails.
...
The Matlab script corresponding to this section is \texttt{explicit\_\-simplex\_\-example.m}, which is in the folder \texttt{tutorial\_examples}. You may copy and paste commands from this script into the Matlab window, or you may run the entire script at once with the following command.
...
In order to compute the homology of our complex, we first create an object that will perform the computation. The following line obtains the default algorithm for performing simplicial homology. There are other variants on the persistence algorithm, and one can also change the ground field. This default object will perform the homology computation over $\mathbb{Z}/2\mathbb{Z}$. The input parameter 3 indicates that homology will be computed in dimensions 0, 1, and 2 --- that is, in all dimensions strictly less than 3.
...
>> stream.finalizeStream(); % explain this command better, Answer: see below
}\end{quote}

In the above, the \texttt{finalizeStream} function is used to ensure that the stream has been fully constructed and is ready for consumption by a persistence algorithm. Note that it can be omitted in the case where the additions are done in increasing order. However, it should be used in general.
...
The Matlab script corresponding to this section is \texttt{house\_example.m}.\\
...
Let's build a stream with nontrivial filtration times. We build a house, with the vertices and edges on the square appearing at time 0, with the top vertex appearing at time 1, with the roof edges appearing at times 2 and 3, and with the roof 2-simplex appearing at time 7.
\begin{quote} \texttt{>> stream = api.Plex4.createExplicitSimplexStream(); \\
...
An important remark is that the methods \texttt{addElement} and \texttt{removeElementIfPresent} % perhaps removeElementIfPresent also removes any coface simplices? I haven't checked. Answer: No it doesn't. I like the remark on the next line.
do not necessarily enforce the definition of a stream. They allow us to build inconsistent complexes in which some simplex $\sigma \in X(t)$ contains a subsimplex $\sigma' \notin X(t)$, meaning that $X(t)$ is not a simplicial complex. The method \texttt{validateVerbose} returns \texttt{1} if our stream is consistent and returns \texttt{0} with explanation if not. 
...
%In \S \ref{sfpc} we will create simplex streams that are not also explicit simplex streams. To display or edit such streams, we will first need to use the method \texttt{makeExplicit}. See Exercise \ref{ripsExpl}. \\
% TODO
...
>> stream = api.Plex4.createVietorisRipsStream(point\_cloud, \-max\_dimension, \-max\_\-filtration\_\-value, \-num\_divisions); % overflow
} \end{quote}
...
\begin{quote} \texttt{>> api.Plex4.createBarcodePlot(filtration\_value\_intervals, 'ripsHouse', max\_\-filtration\_\-value)} \end{quote}
...
{\em Remark.} We can build Vietoris--Rips streams not only on top of Euclidean point clouds, but also on top of explicit metric spaces. For example, if \texttt{m\_space} is an explicit metric space, then we may call a command such as
\begin{quote} \texttt{>> stream = api.Plex4.createVietorisRipsStream(m\_space, max\_dimension, max\_\-filtration\_\-value, num\_\-divisions); \\ % overflow
} \end{quote}
...
% Is the above exercise no longer necessary, or no longer possible? Answer: At the moment it isn't possible. Do you think it is meaningful?
...
\\
>> stream = api.Plex4.createVietorisRipsStream(point\_cloud, max\_dimension, max\_\-filtration\_\-value, num\_\-divisions); \\ % overflow
>> stream.getSize() \\
ans = 82479 \\
\\
...
>> filtration\_index\_intervals = persistence.computeIntervals(stream); \\
>> filtration\_value\_intervals = stream.transform(filtration\_index\_intervals); \\
>> api.Plex4.createBarcodePlot(filtration\_value\_intervals, 'ripsTorus', max\_\-filtration\_\-value)
} \end{quote}
...
\begin{exercise}
Slowly increase the values for $t_{max}$, $d_{max}$ and note how quickly the size of the Vietoris--Rips stream and the time of computation grow. Either increasing $t_{max}$ from 0.9 to 1 or $d_{max}$ from 3 to 4 each roughly doubles the size of the Vietoris--Rips stream. \\
\end{exercise}
...
We create a point cloud of 1,000 points from a figure eight.
\begin{quote} \texttt{>> point\_cloud = examples.PointCloudExamples.getRandomFigure8Points(1000);
} \end{quote}
...
{\em Torus example.} The Matlab script corresponding to this example is \texttt{witness\_example.m}.\\

Let's build a witness stream instance for $100,000$ random points from the unit torus $S^1 \times S^1$ in $\mathbb{R}^4$, with 50 random landmarks.
...
The next command returns the landmark covering measure \texttt{R} from \S \ref{lands}.  Often the value for $t_{max}$ is chosen in proportion to \texttt{R}.
\begin{quote} \texttt{>> R = landmark\_selector.getMaxDistanceFromPointsToLandmarks() \\
R = 1.2344 \hspace{40mm} \% Generally close to 1.2 \\
>> max\_filtration\_value = R / 8; \\
} \end{quote}
% The documentation for getMaxDistanceFromPointsToLandmarks() currently looks incorrect.

We create the witness stream.
\begin{quote} \texttt{>> stream = api.Plex4.createWitnessStream(landmark\_selector, max\_dimension, max\_\-filtration\_\-value, num\_\-divisions); \\ % overflow
>> size = stream.getSize() \\
...
>> filtration\_index\_intervals = persistence.computeIntervals(stream); \\
>> filtration\_value\_intervals = stream.transform(filtration\_index\_intervals); \\
>> api.Plex4.createBarcodePlot(filtration\_value\_intervals, 'witnessTorus', max\_\-filtration\_\-value)
} \end{quote}
...
The witness stream above contains approximately 2,300 simplices, fewer than the approximately 80,000 simplices in the Vietoris--Rips stream from the torus example in \S \ref{Vietoris--Rips}. This is despite the fact that we started with 100,000 points in the witness case, but only 400 points in the Vietoris--Rips case. This supports our belief that the witness stream returns good results at lower computational expense. \\
...
\begin{itemize}
\item{the vertex set is $L$.}
\item{edge $[ab]$ is in $LW_\nu(Z,L,t)$ if there is a witness $z \in Z$ such that $\max\bigl\{d(a,z), \ d(b,z)\bigr\} \leq t+m(z)$.}
...
{\em Sphere example.} The Matlab script corresponding to this example is \texttt{lazy\_witness\_example.m}.\\

We use parameter $\nu = 1$.
\begin{quote} \texttt{>> max\_dimension = 3; \\
>> num\_points = 1000; \\
>> num\_landmark\_points = 100; \\
>> max\_filtration\_value = 0.4; \\
>> nu = 1; \\
% error - int nu is being interpreted as num_divisions!! Having trouble
% using both nu and num_divisions as inputs. 
\\
>> point\_cloud = examples.PointCloudExamples.getRandomSpherePoints(num\_points, max\_dimension - 1); \\
>> landmark\_selector = api.Plex4.createRandomSelector(point\_cloud, num\_\-landmark\_\-points);
} \end{quote}
By default, we set the number of division $N$ to be 20.
\begin{quote} \texttt{>> stream = api.Plex4.createLazyWitnessStream(landmark\_selector, max\_dimension, max\_\-filtration\_\-value, nu); \\
>> size = stream.getSize() \\
ans = 124055 \\
>> persistence = api.Plex4.getDefaultSimplicialAlgorithm(max\_dimension); \\
>> filtration\_index\_intervals = persistence.computeIntervals(stream); \\
>> filtration\_value\_intervals = stream.transform(filtration\_index\_intervals); \\
>> api.Plex4.createBarcodePlot(filtration\_value\_intervals, 'lazySphere', max\_\-filtration\_\-value) \\
} \end{quote}
...
%\begin{quote} \texttt{>> L = WitnessStream.makeRandomLandmarks(pdata, $numLands$); 
%} \end{quote}
%\hspace{55mm} \texttt{\% Or, sequential maxmin landmarks}
%\begin{quote} \texttt{>> R = WitnessStream.estimateRmax(pdata, L); \\
...
We now do an example with real data. The corresponding Matlab script is \texttt{image\_patch\_examples.m}, and it relies on the files \texttt{pointsRange.mat} and \texttt{dct.m}. \\
...
We create a  instance using subclass EuclideanArrayData. We pick 50 sequential maxmin landmark points, find the value of \texttt{R}, and build the lazy witness stream with parameter $\nu = 1$. 
\begin{quote} \texttt{>> pdataRange = EuclideanArrayData(pointsRange); \\
...
\begin{quote} \texttt{>> plot(pointsRangeDct(:,1), pointsRangeDct(:,5), '.'), axis square
} \end{quote}
...
\begin{quote} \texttt{>> eulerCharacteristic(s6) \\
