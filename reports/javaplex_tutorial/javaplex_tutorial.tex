...
% TODO: Add more figures with data points in blue and landmarks in red, such as the one in exercise_5_4_6, to this tutorial.
...
% Change exercise numbering from 3.2.2 to #16?
% Add numbers to examples?
...
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the unit sphere square $S^2 \subset \mathbb{R}^3$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the projective plane. Create an explicit metric space from this distance matrix. 
...
The first part of this exercise is the same as Exercise \ref{Ex:quotProjPlane}. 
...
\begin{exercise}\label{Ex:doubleTorus}
...
\begin{exerciseSol}[\ref{Ex:quotProjPlane}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the unit sphere square $S^2 \subset \mathbb{R}^3$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the projective plane. Create an explicit metric space from this distance matrix. 
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_4\_2\_3.m} and the Matlab function \texttt{projPlaneDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{exerciseSol}[\ref{Ex:quotProjPlaneLazy}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the unit sphere square $S^2 \subset \mathbb{R}^3$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the projective plane. Create an explicit metric space from this distance matrix. 
...
\begin{solution}
See the Matlab script \texttt{exercise\_5\_4\_6.m} and the Matlab function \texttt{projPlaneDistanceMatrix.m} for a solution.
\end{solution}
...
\begin{exerciseSol}[\ref{Ex:doubleTorus}]
Sample points from an embedding of a double torus, that is, a surface of genus two, in $\mathbb{R}^3$. Build a lazy witness stream on this Euclidean metric space. Confirm the barcodes match the homology of a double torus. Choosing suitable parameters will not be easy.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_5\_4\_7.m} and the Matlab function \texttt{getDoubleTorusPoints.m}. Thanks to Ulrich Bauer for this solution.
\end{solution}
