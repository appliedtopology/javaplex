...
\begin{quote} \texttt{>> cd tutorial\_examples
} \end{quote}
...
>> scatter(point\_cloud(:,1), point\_cloud(:,2), '.')\\
>> axis equal\\
} \end{quote}
...
We plot the points.
\begin{quote} \texttt{>> figure\\
>> scatter3(point\_cloud(:,1), point\_cloud(:,2), point\_cloud(:,3), '.')\\
>> axis equal\\
} \end{quote}
...
\begin{quote} \texttt{>> figure\\
>> scatter(point\_cloud(:,3), point\_cloud(:,4), '.')\\
>> axis equal\\
} \end{quote}
...
\begin{quote} \texttt{>> persistence = api.Plex4.getDefaultSimplicialAlgorithm(max\_dimension); \\
>> filtration\_index\_intervals = persistence.computeIntervals(stream); \\
>> filtration\_value\_intervals = stream.transform(filtration\_index\_intervals); \\ % TODO: missing semicolon in matlab script
...
\begin{quote} \texttt{>> scatter(pointsRangeDct(:,1), pointsRangeDct(:,5), '.') \\
axis square
} \end{quote}
