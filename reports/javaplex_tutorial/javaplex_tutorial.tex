...
In Matlab, change directories to folder \texttt{matlab-examples}. Run the \texttt{load\_javaplex.m} file. 
...
Your output should be the same except for the last few characters. \\% Andrew, is there a better command users can use to test their installation is correct?
...
%\begin{quote} \texttt{>> pdataHouse.dimension\\
%ans = 2
%} \end{quote}
...
%\begin{quote} \texttt{>> pdataHouse.coordinate(5, 0)\\
%ans = 0 \\
...
{\em Torus example.} Try the following sequence of commands. We select 400 points from a $20 \times 20$ grid on the unit torus $S^1 \times S^1$ in $\mathbb{R}^4$. % If I select points randomly from the torus instead from the regular grid, then the Vietoris--Rips stream does not seem to be able to recover the correct Betti numbers.
A small amount of noise is added to each point. We build the Vietoris--Rips stream.
\begin{quote} \texttt{>> max\_dimension = 3; \\
...
\begin{exercise}
Slowly increase the values for $t_{max}$, $d_{max}$ and note how quickly the size of the Vietoris--Rips stream and the time of computation grows. Separately increasing $t_{max}$ from 0.9 to 1, $d_{max}$ from 3 to 4 each roughly doubles the size of the Vietoris--Rips stream. \\
\end{exercise}
...
{\em Remark.} We can select landmark pionts not only from Euclidean point clouds, but also from explicit metric spaces. For example, if \texttt{m\_space} is an explicit metric space, then we may call a command such as
\begin{quote} \texttt{>> maxmin\_selector = api.Plex4.createMaxMinSelector(m\_space, num\_landmark\_points); \\
} \end{quote}

% Do an example such as the one below?

%{\em Case DistanceData.} Recall the house example.
%\begin{quote} \texttt{>> pdataHouseDD = DistanceData(distances);
%} \end{quote}
%We select 3 sequential maxmin landmarks.
%\begin{quote} \texttt{>> L = maxminLandmarks(distances, 3, 'd') \\
%L =
%} \end{quote}
