...
\author{Henry Adams}
\email[H.~Adams]{henrya@math.stanford.edu}
...
% The first line below must precede the second and third lines, because otherwise the spacing in the table of contents is increased by too much.
\tableofcontents
\setlength{\parindent}{0pt}
\setlength{\parskip}{8pt}

% TODO: add example computing a representative cycle for each barcode
% TODO: add representative cycles example which Andrew has made to tutorial

% TODO: To encourage good habits, use finalizeStream() on all ExplicitStreams (not just the ones created out of order, where it is necessary).

% TODO: add comment about javaPlex in Java (without Matlab)
...
% TODO: tutorial is not up-to-date with Downloads section of javaPlex site. Run "ant"

% TODO: standardize spacing for Matlab output

% TODO: standardize spacing for "%" comments in verbatim sections

% TODO: standardize line breaks (overflow) in verbatim sections. MVJ suggests using ...

% TODO: standardize citations in tutorial wiki

% TODO: The links to the tutorial from my webpage and from the MRC website are to the pre-Snowbird 4.0.0 version. The link from the tutorial wiki is to the latest version.

% TODO: add endpoint extraction example (as Matlab matrix) which Andrew has made to tutorial.

% TODO: put note in JPlex tutorial pointing to this new tutorial.
...
Please email Henry at \texttt{henrya@math.stanford.edu} or Andrew at \texttt{atausz@stanford.edu} if you have questions about this tutorial.
...
To install javaPlex for Matlab, go the the website \url{http://code.google.com/p/javaplex/downloads/list}. Download the zip file containing the Matlab examples. It should be called something like \texttt{matlab-examples\-4.0.0.tar.gz}. Extract the zip file. The resulting folder should be called \texttt{matlab-examples}. 
...
The following Matlab scripts containing the commands in this tutorial are available in the folder \texttt{matlab\_examples/tutorial\_examples}. This means that you don't need to type in each command individually. % Add note? DONT copy and paste commands from pdf
\begin{itemize}
...
\item \texttt{exercise\_4\_2\_1.m}
\item \texttt{exercise\_4\_2\_2.m}
\item \texttt{exercise\_5\_1\_2.m}
\item \texttt{exercise\_5\_1\_3.m}
\item \texttt{exercise\_5\_4\_4.m}
\item \texttt{exercise\_5\_4\_5.m}
...
\begin{exercise}\label{Ex:projPlane}
...
\begin{exercise}\label{Ex:ripsSize}
...
\begin{exercise}\label{Ex:ripsPlanarBetti2}
...
\begin{exercise}\label{Ex:ripsPlanarBetti6}
...
\begin{exercise}\label{Ex:landmarksHouse}
...
\begin{exercise}\label{Ex:landmarksR}
...
% point_cloud = ...
% examples.PointCloudExamples.getRandomSphereProductPoints(num_points, 1, 2);
...
%stream = api.Plex4.createWitnessStream(landmark_selector, ...
%                                           max_dimension, ...
...
\begin{exercise}\label{Ex:lazyHouseNu1}
...
\begin{exercise}\label{Ex:lazyHouseNu_0_2}
Repeat the above exercise with $\nu = 0$ and with $\nu = 2$. 
\end{exercise}

\begin{exercise}\label{Ex:lazyFlag}
...
The first part of this exercise is the same as Exercise \ref{Ex:flatTorus}. 
...
Build a lazy witness stream on this explicit metric space with 50 landmarks chosen via sequential maxmin and with $\nu = 1$.  Confirm the barcodes match the homology of a torus. 
\end{exercise}
...
% Add an exercise in which you sample points from HALF of a sphere in R^3. Then, build the metric space for RP^2 (proj plane) by taking the distance from x and y to be the min distance of x to y and -y. This was done by David Eisenbud and Agnes Beadry at MRC.
...
\begin{exerciseSol}[\ref{Ex:torus}]
Build a simplicial complex homeomorphic to the torus. Compute its Betti numbers. {\em Hint:} You will need at least 7 vertices \cite[page 107]{h02}. We recommend using a $3\times 3$ grid of 9 vertices.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_3\_1\_1.m} in folder \texttt{tutorial\_solutions} for a solution. We use 9 vertices, which we think of as a $3 \times 3$ grid numbered as a telephone keypad. We identify opposite sides.
\end{solution}
...
\begin{exerciseSol}[\ref{Ex:klein}]
Build a simplicial complex homeomorphic to the Klein bottle. Check that it has the same Betti numbers as the torus over $\mathbb{Z}_2$ coefficients but different Betti numbers over $\mathbb{Z}_3$ coefficients.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_3\_1\_2.m} for a solution. We use 9 vertices, which we think of as a $3 \times 3$ grid numbered as a telephone keypad. We identify opposite sides, with left and right sides identified with a twist. 
\end{solution}
...
\begin{exerciseSol}[\ref{Ex:projPlane}]
Build a simplicial complex homeomorphic to the projective plane. Find its Betti numbers over $\mathbb{Z}_2$ and $\mathbb{Z}_3$ coefficients.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_3\_1\_3.m} for a solution. We use the minimal triangulation for the projective plane, which contains 6 vertices.
\end{solution}
...
\begin{exerciseSol}[\ref{Ex:flatTorus}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the square $[0, 1] \times [0, 1]$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the flat torus. Create an explicit metric space from this distance matrix.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_4\_2\_1.m} and the Matlab function \texttt{flatTorusDistanceMatrix.m} for a solution.
\end{solution}

\begin{exerciseSol}[\ref{Ex:flatKlein}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the square $[0, 1] \times [0, 1]$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the flat Klein bottle. Create an explicit metric space from this distance matrix.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_4\_2\_2.m} and the Matlab function \texttt{flatKleinDistanceMatrix.m} for a solution.
\end{solution}

\begin{exerciseSol}[\ref{Ex:ripsSize}]
Slowly increase the values for $t_{max}$, $d_{max}$ and note how quickly the size of the Vietoris--Rips stream and the time of computation grow. Either increasing $t_{max}$ from 0.9 to 1 or increasing $d_{max}$ from 3 to 4 roughly doubles the size of the Vietoris--Rips stream.
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:ripsPlanarBetti2}]
Find a planar dataset $Z \subset \mathbb{R}^2$ and a filtration value $t$ such that $\VR(Z,t)$ has nonzero $Betti_2$. Build a Vietoris--Rips stream to confirm your answer.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_5\_1\_2.m} for a solution. Our planar dataset is 6 evenly spaced points on the unit circle. We build a Vietoris--Rips stream which, at the correct filtration value, is an octahedron. 
\end{solution}

\begin{exerciseSol}[\ref{Ex:ripsPlanarBetti6}]
Find a planar dataset $Z \subset \mathbb{R}^2$ and a filtration value $t$ such that $\VR(Z,t)$ has nonzero $Betti_6$. When building a Vietoris--Rips stream to confirm your answer, don't forget to choose $d_{max} = 7$.
\end{exerciseSol}

\begin{solution}
See the Matlab script \texttt{exercise\_5\_1\_3.m} for a solution. Our planar dataset is 14 evenly spaced points on the unit circle. We build a Vietoris--Rips stream which, at the correct filtration value, is homeomorphic to the 6-sphere. It has 14 vertices because it is obtained by suspending the 0-sphere six times, for a total of $2 + (6 \times 2) = 14$ vertices. 
\end{solution}

\begin{exerciseSol}[\ref{Ex:landmarksHouse}]
Let $Z$ be the point cloud in Figure \ref{fig:housePointCloud} from Section \ref{euc}, corresponding to the house point cloud. Suppose we are using sequential maxmin to select a set $L$ of 3 landmarks, and the first (randomly selected) landmark is $(1,0)$. Find by hand the other two landmarks in $L$.
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:landmarksR}]
Let $Z$ be a point cloud and $L$ a landmark subset. Show that if $L$ is chosen via sequential maxmin, then for any $l_i,l_j\in L$, we have $d(l_i,l_j)\geq\texttt{R}$.
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:witnessHouse}]
Let $Z$ be the point cloud in Figure \ref{fig:housePointCloud} from Section \ref{euc}, corresponding to the house point cloud. Let $L = \{(1,0),(0,3),(-1,0)\}$ be the landmark subset. Find by hand the filtration time for the edge between vertices $(1,0)$ and $(0,3)$. Which point or points witness this edge? What is the filtration time for the lone 2-simplex $[(1,0),(0,3),(-1,0)]$?
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:lazyHouseNu1}]
Let $Z$ be the point cloud in Figure \ref{fig:housePointCloud} from Section \ref{euc}, corresponding to the house point cloud. Let $L = \{(1,0),(0,3),(-1,0)\}$ be the landmark subset. Let $\nu = 1$. Find by hand the filtration time for the edge between vertices $(1,0)$ and $(0,3)$. Which point or points witness this edge? What is the filtration time for the lone 2-simplex $[(1,0),(0,3),(-1,0)]$?
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:lazyHouseNu_0_2}]
Repeat the above exercise with $\nu = 0$ and with $\nu = 2$.
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:lazyFlag}]
Check that the 1-skeleton of a witness complex $\W(Z,L,t)$ is the same as the 1-skeleton of a lazy witness complex $\LW_2(Z,L,t)$. As a consequence, $\LW_2(Z,L,t)$ is the flag complex of $\W(Z,L,t)$.
\end{exerciseSol}
...
\begin{exerciseSol}[\ref{Ex:flatTorusLazy}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the square $[0, 1] \times [0, 1]$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the flat torus. Create an explicit metric space from this distance matrix. 
...
\begin{solution}
See the Matlab script \texttt{exercise\_5\_4\_4.m} and the Matlab function \texttt{flatTorusDistanceMatrix.m} for a solution.
\end{solution}

\begin{exerciseSol}[\ref{Ex:flatKleinLazy}]
Write a Matlab script (or function---see Section \ref{matlabFunctions}) that first selects 1,000 random points from the square $[0, 1] \times [0, 1]$ and then computes the 1,000 $\times$ 1,000 distance matrix for these points under the induced metric on the flat Klein bottle. Create an explicit metric space from this distance matrix. 
...
\begin{solution}
See the Matlab script \texttt{exercise\_5\_4\_5.m} and the Matlab function \texttt{flatKleinDistanceMatrix.m} for a solution.
\end{solution}
...
\bibitem{cidsz08}
{\sc G.~Carlsson, T.~Ishkhanov, V.~de Silva, and A.~Zomorodian}, {\em On the local behavior of spaces of natural images}, Int. J. Computer Vision, 76 (2008), pp.~1--12.
