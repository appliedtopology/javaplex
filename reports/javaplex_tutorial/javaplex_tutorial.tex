...
Matlab scripts containing the commands in this tutorial are available in the folder \texttt{matlab\_examples/tutorial\_examples}. This means you don't need to type in each command individually. The command scripts are the following.
\begin{itemize}
\item \texttt{core\_subsets\_example.m}
\item \texttt{euler\_characteristic\_example.m}
\item \texttt{explicit\_metric\_space.m}
\item \texttt{explicit\_simplex\_example.m}
\item \texttt{house\_example.m}
\item \texttt{image\_patch\_examples.m}
\item \texttt{landmark\_example.m}
\item \texttt{lazy\_witness\_example.m}
\item \texttt{pointcloud\_examples.m}
\item \texttt{rips\_example.m}
\item \texttt{witness\_example.m}
\end{itemize}
This tutorial also uses the following Matlab functions
\begin{itemize}
\item \texttt{coreSubset.m}
\item \texttt{dct.m}
\item \texttt{eulerCharacteristic.m}
\end{itemize}
and following Matlab data files, also available in the folder \texttt{matlab\_examples/tutorial\_examples}.
\begin{itemize}
...
% add solutions pdf when it gets made.
...
\subsection{Explicit simplex streams and persistent homology} Let's build a stream with nontrivial filtration times. \\
...
{\em House example.} Let's build a Vietoris--Rips stream from the house point cloud in \S \ref{euc}. Note this stream is different than the explicit stream we built in \S \ref{explicitStreamPers}.
...
{\em Torus example.} Let's build a witness stream instance for $100,000$ random points from the unit torus $S^1 \times S^1$ in $\mathbb{R}^4$, with 50 random landmarks.
...
The witness stream above contains approximately 2,000 simplices, fewer than the approximately 80,000 simplices in the Vietoris--Rips stream from the torus example in \S \ref{Vietoris--Rips}. This is despite the fact that we started with a point cloud of 100,000 points in the witness case, but of only 400 points in the Vietoris--Rips case. This supports our belief that the witness stream returns good results at lower computational expense. \\
...
>> stream = streams.impl.LazyWitnessStream(landmark\_selector.getUnderlyingMetricSpace(), landmark\_selector, max\_dimension, max\_filtration\_value, nu, num\_divisions); \\
