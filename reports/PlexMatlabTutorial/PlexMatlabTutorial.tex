...
\noindent If you are interested in JPlex, then you may also be interested in two other software packages that compute persistent homology. The first is Dionysus (\url{http://www.mrzv.org/software/dionysus}) by Dmitriy Morozov, and the second is JavaPlex (\url{http://code.google.com/p/javaplex}) by Tim Harrington, Andrew Tausz, and Mikael Vejdemo Johansson. \\




\subsection{Accompanying files}\label{accompany}
This tutorial should be accompanied by the following files, available at the TMSCSCS website above.
\begin{enumerate}
\item \texttt{betti2vect.m}
\item \texttt{commandListMatlab.rtf}
\item \texttt{coreSubset.m}
\item \texttt{dct.m}
\item \texttt{eulerCharacteristic.m}
\item \texttt{exerciseAnswersFigures.pdf}
\item \texttt{exerciseAnswersMatlab.rtf}
\item \texttt{interval2mat.m}
\item \texttt{kDensitySlow.m}
\item \texttt{maxminLandmarks.m}
\item \texttt{PlexMatlabTutorial.pdf}
\item \texttt{pointsFigure8.m}
\item \texttt{pointsRange.mat}
\item \texttt{pointsTorus.m}
\item \texttt{startJPlex.m}
\end{enumerate}
PDF file (11) is this tutorial. Text file (2) lists all commands in this tutorial, so that you may copy and paste commands into the Matlab window, or into a Matlab m-file script. Text file (7) has answers for the exercises in this tutorial, and PDF file (6) contains some figures to accompany the answers. \\

\noindent The remaining files are Matlab files. M-file (15) is a script: it is simply a shortcut that calls a sequence of Matlab commands. M-files (1), (3)-(5), (8)-(10), (12), and (14) are functions: they accept input and produce output. Writing your own such scripts or functions can be very useful. MAT-file (13) contains data. \\
...
\noindent Copy Matlab files (1), (3)-(5), (8)-(10), and (12)-(15) from \S \ref{accompany} into the current Matlab directory. \\

\noindent Copy \texttt{plex.jar} into a directory of your choice. If you choose the current Matlab directory, then you do not need to edit m-file \texttt{startJPlex.m} and can skip the rest of this paragraph. Suppose instead you have chosen the directory \texttt{/Users/myName/}. Open the m-file \texttt{startJPlex.m}, which contains the following lines.
...
\noindent Run the \texttt{startJPlex.m} file. 
...
\begin{quote} \texttt{>> Simplex.makePoint(1, 2) \index{1}{Simplex.makePoint}\\
ans = <(2) 1> \hspace{30mm} \% You should get this answer
} \end{quote}

\noindent Each time upon starting a new Matlab session, you will need to run \texttt{startJPlex.m}.   \\

\noindent Note: for fairly trivial technical reasons, the version of \texttt{plex.jar} that you downloaded from the TMSCSCS website will not give you the correct answers for the circle and 6-sphere examples in \S \ref{SS:explicit}. Please email Henry at \url{henrya@math.stanford.edu} to get a different plex.jar file which will give you the correct answers for these (and all other) examples.
\\
...
\noindent Below is a brief math review. For more details, see \cite{a83, eh10, h02, zc05}. 
...
\noindent Persistent homology depends heavily on functoriality: for $t\leq s$, the inclusion $i:X(t)\to X(s)$ of simplicial complexes induces a map $i_*:H_k(X(t))\to H_k(X(s))$ between homology groups. \\
...
In JPlex, filtered simplicial complexes are called streams and are implemented by the class SimplexStream. The subclass ExplicitStream allows us to build a SimplexStream instance from scratch. In \S \ref{sfpc} we will learn about subclasses RipsStream, WitnessStream, and LazyWitnessStream, which construct SimplexStream instances from point cloud data. \\

\subsection{Subclass ExplicitStream and homology}\label{SS:explicit}
Since JPlex is designed to compute persistent homology, it is not the most efficient software (notationally or computationally) for computing homology. \\

\begin{wrapfigure}{r}{1.5in}
	\begin{center}
   	\includegraphics[width=1.5in]{s1.pdf}
  	\end{center}
\end{wrapfigure}

\noindent {\em Circle example.} Let's build a simplicial complex homeomorphic to a circle. We have three 0-simplices: [1], [2], and [3], and three 1-simplices: [12], [23], [31]. To build a simplicial complex in JPlex we simply build a stream in which all filtration times are zero. First we get an empty ExplicitStream instance.
\begin{quote} \texttt{>> s1 = ExplicitStream; \index{1}{ExplicitStream}
} \end{quote}
Many command lines in this tutorial will end with a semicolon to supress unwanted output as below. 
\begin{quote} \texttt{>> s1 = ExplicitStream \index{1}{ExplicitStream}\\
s1 = edu.stanford.math.plex.ExplicitStream@b138fc
} \end{quote}

\noindent Next we add simplices using the method \texttt{add}. Two inputs are required: a matrix whose rows contain the simplices to be added, and a vector listing the corresponding filtration times. We choose all filtration times to be zero as we are building a simplicial complex instead of a stream.
\begin{quote} \texttt{>> s1.add([1;2;3], [0;0;0]) \hspace{22mm} \% adds 3 vertices: [1], [2], and [3] \index{1}{add}
>> s1.add([1,2;2,3;3,1], [0;0;0]) \hspace{11mm} \% adds 3 edges: [12], [23], and [31]
} \end{quote}

\noindent Let's inspect what we've built. The object \texttt{s1.dump($k$)} contains information about the $k$-simplices of \texttt{s1}. We display the 1-simplices and their filtration times.
\begin{quote} \texttt
{>> s1.dump(1).C \hspace{30mm} \% C stands for cell \index{1}{dump}\\
ans =
} \end{quote}

\hspace{15mm} \texttt{1\ \ \ 2}

\hspace{15mm} \texttt{1\ \ \ 3}

\hspace{15mm} \texttt{2\ \ \ 3}
\begin{quote} \texttt{>> s1.dump(1).F \hspace{30mm} \% F stands for filtration \index{1}{dump}\\
ans =
} \end{quote}
...
\noindent Our \texttt{s1}, like any ExplicitStream instance, has two states: open or closed. The state is automatically set to open whenever we edit or view the simplices of \texttt{s1}. We must manually close \texttt{s1} before computing its homology.
\begin{quote} \texttt{>> s1.close \index{1}{close}
} \end{quote}
The strange command names for computing homology will make sense later when we compute persistent homology.
\begin{quote} \texttt{>> intervals = Plex.Persistence.computeIntervals(s1); \index{1}{Plex.Persistence.computeIntervals}\\
>> Plex.FilterInfinite(intervals) \index{1}{FilterInfinite}\\
ans = BN\{1, 1\}
} \end{quote} 
The result \texttt{BN\{1, 1\}} means that \texttt{s1} has $Betti_0 = 1$ and $Betti_1 = 1$, which are the Betti numbers of a circle.
%%%%%%%%%%%%%%%%%%%%%%%%%
(If you instead get BN\{1\}, email Henry at \url{henrya@math.stanford.edu} to get a different plex.jar file.) \\
%%%%%%%%%%%%%%%%%%%%%%%%%

\noindent {\em 6-sphere example.} Let's build a 6-sphere, which is homeomorphic to the boundary of a 7-simplex. Adding the simplices manually as we did in the circle case would be very tedius: there are 8 vertices, 28 edges, 56 triangles,  etc. Instead, we start with a 7-simplex that has no faces. We add its faces using the method \texttt{ensure\_all\_faces}. Then, we remove the 7-simplex, leaving only its boundary.
\begin{quote} \texttt{>> s6 = ExplicitStream; \index{1}{ExplicitStream}\\
>> s6.add(1:8, 0) \hspace{20mm} \% 1:8 is the matrix [1,2,...,8] \index{1}{add}
} \end{quote}
\hspace{67mm}\texttt{\% adds the 7-simplex}
\begin{quote} \texttt{>> s6.ensure\_all\_faces \index{1}{ensure\_all\_faces}\\
>> s6.remove(1:8) \index{1}{remove}
} \end{quote}
How many 3-simplices did we avoid adding manually?
\begin{quote} \texttt{>> size(s6.dump(3).C) \index{1}{dump}\index{1}{size}\\
ans = 70 \quad 4 \hspace{30mm} \% 70 3-simplices
} \end{quote}

\noindent We compute the homology.
\begin{quote} \texttt{>> s6.close \index{1}{close}\\
>> intervals = Plex.Persistence.computeIntervals(s6); \index{1}{Plex.Persistence.computeIntervals}\\
>> Plex.FilterInfinite(intervals) \index{1}{FilterInfinite}\\
ans = BN\{1, 0, 0, 0, 0, 0, 1\}
} \end{quote}
We get nonzero Betti numbers $Betti_0 = Betti_6 = 1$. 
%%%%%%%%%%%%%%%%%%%%%%%%%
(If you instead get BN\{1\}, email Henry at \url{henrya@math.stanford.edu} to get a different plex.jar file.) \\
%%%%%%%%%%%%%%%%%%%%%%%%%

\noindent The following command tells us that we have been computing homology over the coefficient field $\mathbb{Z}_{11}$. 
\begin{quote} \texttt{>> Persistence.baseModulus \index{1}{Persistence.baseModulus}\\
ans = 11
} \end{quote}
We can instead compute over modulus 13 (or any other prime between 2 and 251).
\begin{quote} \texttt{>> Persistence.setBaseModulus(13) \index{1}{Persistence.setBaseModulus}\\
} \end{quote}

\begin{exercise}
Build a simplicial complex homeomorphic to the torus. Compute its Betti numbers. {\em Hint:} You will need at least 7 vertices \cite[page 107]{h02}. I recommend using a $3\times 3$ grid of 9 vertices. \\
\end{exercise}
...
\noindent Or, using the m-file \texttt{interval2mat.m}, we can display all of the intervals at once.
...
\noindent One $Betti_0$ interval and one $Betti_1$ interval are semi-infinite.
\begin{quote} \texttt{>> Plex.FilterInfinite(intervals) \index{1}{FilterInfinite}\\
ans = BN\{1, 1\} \\
} \end{quote}

\noindent The method \texttt{Plex.plot} lets us display the intervals as a Betti barcode. The three inputs are \texttt{intervals}, a string to appear as a label, and the maximum filtration time (which may be adjusted $\pm 10\%$) for the plot. 
...
\noindent The filtration times are on the horizontal axis. The $Betti_k$ number of the stream at filtration time $t$ is the number of intervals in the dimension $k$ plot that intersect a vertical line through $t$. Check that the displayed intervals agree with the filtration times we built into the stream \texttt{house}. At time 0, a connected component and a 1-dimensional hole form. At time 1, a second connected component appears, which joins to the first at time 2. A second 1-dimensional hole forms at time 3, and closes at time 7. \\

\noindent The method \texttt{Plex.scatter} displays the same information as \texttt{Plex.plot} but in a different format: an interval starting at $start$ and ending at $end$ is plotted as the point $(start,end)$ in $\mathbb{R}^2$, which is necessarily above the diagonal.
...
\noindent The methods \texttt{add} and \texttt{remove} do not necessarily enforce the definition of a stream. They allow us to build inconsistent streams in which some simplex $\sigma \in X(t)$ contains a subsimplex $\sigma' \notin X(t)$, meaning that $X(t)$ is not a simplicial complex. The method \texttt{verify(1)} returns \texttt{1} if our stream is consistent and returns \texttt{0} with explanation if not. 
...
\noindent In \S \ref{sfpc} we will create SimplexStream instances that, unlike \texttt{house}, are not also ExplicitStream instances. To display or edit such streams, we will first need to use the method \texttt{makeExplicit}. See Exercise \ref{ripsExpl}. \\
...
\noindent Let's give coordinates to the points of our house.
...
\noindent We create a PointData instance using these coordinates. The input to the EuclideanArrayData constructor is a matrix whose $i$-th row lists the coordinates of the $i$-th point.
\begin{quote} \texttt{>> pdataHouse = EuclideanArrayData([-1,0;1,0;1,2;-1,2;0,3]); \index{1}{EuclideanArrayData}\\
} \end{quote}

\noindent Any PointData instance can display the number of data points and the distance between, say, points 1 and 3.
...
\noindent The m-files \texttt{pointsFigure8.m} and \texttt{pointsTorus.m} each create a matrix of points which can be input into subclass EuclideanArrayData. \\

\noindent {\em Figure 8 example.} The first command below selects 100 points randomly from a figure eight (the union of unit circles centered at (0,1) and (0,-1)), and then adds noise to each point. The second command plots these points. The third command creates a PointData instance.
...
\noindent The matrix \texttt{distances} summarizes the metric for our house points in Figure \ref{fig:pdataHouse}: entry $(i,j)$ is the distance from point $i$ to point $j$. Don't forget you can copy and paste commands from \texttt{commandListMatlab.rtf} into the Matlab window, or enter them using a script! 
...
\noindent Let's build a Rips stream instance \texttt{ripsHouse} from the PointData instance \texttt{pdataHouse}. Note this stream is different than the ExplicitStream \texttt{house} we built in \S \ref{explicitStream}.
...
\noindent We compute and display the Betti intervals. Typically the last input for the method \texttt{Plex.plot} will be $t_{max}$, since there is no reason to display filtration times that we haven't computed.
...
\noindent The second dimensional Betti plot does not appear because there are no $Betti_2$ intervals. Check that these plots are consistent with the Rips definition: edges $[3,5]$ and $[4,5]$ appear at filtration time $t = \sqrt{2}$; the square appears at $t = 2$; the square closes at $t = \sqrt{8}$. \\
...
\noindent {\em Torus example.} Try the following sequence of commands. We select a $20 \times 20$ grid of noisy points from a torus and build the RipsStream \texttt{ripsT20}. The fourth command returns the total number of simplices in \texttt{ripsT20}.
\begin{quote} \texttt{>> pointsT20 = pointsTorus(20); \index{1}{pointsTorus.m}\\
>> pdataT20 = EuclideanArrayData(pointsT20); \index{1}{EuclideanArrayData}\\
...
\noindent There are two common methods for selecting landmark points. The first is to choose the landmarks $L$ randomly from point cloud $Z$. We select 25 random landmarks from figure eight PointData instance \texttt{pdataF100}.
...
\noindent Plot landmark points \texttt{L1} and \texttt{L2} to see the difference between random and sequential maxmin landmark selection.
...
\noindent Sequential maxmin seems to do a better job of choosing landmarks that cover the figure eight and that are spread apart. \\

\noindent {\em Case DistanceData.} Recall the house example.
\begin{quote} \texttt{>> pdataHouseDD = DistanceData(distances); \index{1}{DistanceData}
} \end{quote}
...
\noindent Given point cloud $Z$ and landmark subset $L$, we define $\texttt{R} = \max_{z\in Z}\bigl\{d(z,L)\bigr\}$. Number \texttt{R} reflects how finely the landmarks cover the dataset. We often use it as a guide for selecting the maximum filtration value $t_{max}$ for a WitnessStream or LazyWitnessStream instance. \\
...
\noindent We plot the Betti intervals.
...
\noindent The following sequence of commands is typical.
...
\noindent We create a PointData instance using subclass EuclideanArrayData. We pick 50 sequential maxmin landmark points, find the value of \texttt{R}, and build the lazy witness stream with parameter $\nu = 1$. 
...
\noindent Two of the DCT basis vectors are horizontal and linear gradients.
...
\noindent We plot the projection of \texttt{pointsRangeDct} onto the linear gradient DCT basis vectors.
...
\noindent The projection of $X^5(300,30)$ in Figure (a) shows a circle. It is called the range primary circle and is parameterized in Figure (b).
...
\noindent The following command returns your maximum heap size in bytes.
...
\noindent M-file \texttt{coreSubset.m} builds a core subset. The following command is typical.
...
\noindent {\em Prime numbers example.} The command \texttt{primes(3571)} returns a vector listing all prime numbers less than or equal to 3571, which is the 500-th prime. We think of these primes as points in $\mathbb{R}$ and build the core subset of the 10 densest points with density parameter $k = 1$.
