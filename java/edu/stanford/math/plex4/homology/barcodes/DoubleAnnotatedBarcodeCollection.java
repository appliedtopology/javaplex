 ...
 *
 */
public class DoubleAnnotatedBarcodeCollection<T> {
	private final TIntObjectHashMap<DoubleAnnotatedBarcode<T>> barcodeMap = new TIntObjectHashMap<DoubleAnnotatedBarcode<T>>();
	...
	public void addInterval(int dimension, DoubleHalfOpenInterval interval, T generatingCycle) {
		...
		if (!this.barcodeMap.containsKey(dimension)) {
			this.barcodeMap.put(dimension, new DoubleAnnotatedBarcode<T>(dimension));
		}
	 ...
	 */
	public DoubleAnnotatedBarcodeCollection<T> getInfiniteIntervals() {
		DoubleAnnotatedBarcodeCollection<T> collection = new DoubleAnnotatedBarcodeCollection<T>();
		
		for (TIntObjectIterator<DoubleAnnotatedBarcode<T>> iterator = this.barcodeMap.iterator(); iterator.hasNext(); ) {
			iterator.advance();
	 ...
	 */
	public DoubleAnnotatedBarcode<T> getBarcode(int dimension) {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		DoubleAnnotatedBarcodeCollection<?> other = (DoubleAnnotatedBarcodeCollection<?>) obj;
		if (barcodeMap == null) {
