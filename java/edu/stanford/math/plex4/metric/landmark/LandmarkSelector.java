...
public abstract class LandmarkSelector<T> implements AbstractObjectMetricSpace<T> {
	...
	public T[] getPoints() {
		T[] landmarkPoints = ObjectArrayUtility.createArray(this.indexMapping.length, metricSpace.getPoint(0));
		for (int i = 0; i < this.indexMapping.length; i++) {
			landmarkPoints[i] = this.metricSpace.getPoint(this.indexMapping[i]);
		}
