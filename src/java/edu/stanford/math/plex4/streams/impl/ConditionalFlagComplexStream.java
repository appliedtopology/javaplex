...
/**
 * <p>
 * This class implements a simplex stream which is similar to the
 * FlagComplexStream class, except that higher order simplices are subject to an
 * abstract property.
 * </p>
 * 
 * <p>
 * A motivating example is the regular witness complex, which has the property
 ...
 * outside the landmark set for all of the vertices in the simplex.
 * </p>
 * 
 * @author Andrew Tausz
 */
public abstract class ConditionalFlagComplexStream extends PrimitiveStream<Simplex> {
	...
	protected void addCofaces(UndirectedWeightedListGraph G, int k, Simplex tau, TIntHashSet N, double filtrationValue) {
		...
		if (member.getFirst()) {
			double adjustedFiltrationValue = this.converter.computeInducedFiltrationValue(filtrationValue, member.getSecond());
			this.storageStructure.addElement(newSimplex, this.converter.getFiltrationIndex(adjustedFiltrationValue));
		} else {
			return;
		}
