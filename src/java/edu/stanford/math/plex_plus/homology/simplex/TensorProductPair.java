 ...
 *
 */
public class TensorProductPair<T extends ChainBasisElement, U extends ChainBasisElement> extends GenericPair<T, U> implements ChainBasisElement {

	public TensorProductPair(T first, U second) {
		super(first, second);
	...
	public ChainBasisElement[] getBoundaryArray() {
		...
		for (int i = 0; i < d_a.length; i++) {
			boundary[i] = new TensorProductPair<ChainBasisElement, ChainBasisElement>(d_a[i], this.second);
		}
		
		for (int i = 0; i < d_b.length; i++) {
			boundary[i + d_a.length] = new TensorProductPair<ChainBasisElement, ChainBasisElement>(this.first, d_b[i]);
		}
