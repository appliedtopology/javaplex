...
codomain_points = examples.PointCloudExamples.getOctahedronVertices() + 0;
...
codomain_points = examples.PointCloudExamples.getEquispacedCirclePoints(codomain_size);
codomain_points = codomain_points + ones(codomain_size, 1) * [0, 0];
...
domain_stream = api.Plex4.createLazyWitnessStream(landmark_selector, 2, 0.5, 2);
...
%%
output_map = map;
%output_map = normalize_rows(map);
%output_map = normalize_rows(map')';
%%

dlmwrite('mapping.txt', full(output_map));
dlmwrite('domain_points.txt', domain_points);
dlmwrite('codomain_points.txt', codomain_points);
